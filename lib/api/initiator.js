...
Initiator.prototype.add = function(initiators) {
    ...
    // Remove extension
    initiatorPath = initiatorPath.substring(0, initiatorPath.length - path.extname(initiatorPath).length);
...
Initiator.prototype._getAssetPath = function(initiator) {
    var realPath = path.join(config.pub.dir, config.module.link, this.name, initiator);
    return path.relative(config.pub.dir, realPath);
};

Initiator.prototype._validateInitiator = function(initiator) {
    var assetPath = this._getAssetPath(initiator);
    return fs.existsSync(path.join(config.pub.dir, assetPath));
};
